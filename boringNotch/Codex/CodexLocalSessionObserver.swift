...
struct CodexObservedLifecycleEvent: Equatable, Sendable {
  ...
  func activityEvent(sequence: Int) -> CodexActivityEvent? {
    ...
    let eventKind: CodexActivityEvent.Kind
    switch (kind, isSubagent) {
    case (.working, false): eventKind = .working
    case (.completed, false): eventKind = .completed
    case (.failed, false): eventKind = .failed
    case (.working, true): eventKind = .subagentStarted(agentID: sessionID)
    case (.completed, true), (.failed, true), (.stale, true):
      eventKind = .subagentStopped(agentID: sessionID)
...
struct CodexTranscriptAccumulator: Sendable {
  ...
  mutating func consume(line: Data) -> CodexObservedLifecycleEvent? {
    let isFirstLine = !hasConsumedLine
    hasConsumedLine = true
    guard !line.isEmpty, line.count <= CodexLocalSessionObserver.maximumLineSize,
    ...
    let kind: CodexObservedLifecycleEvent.Kind
    switch eventType {
    ...
    turnID = record.payload.turnID ?? turnID
    return CodexObservedLifecycleEvent(
      kind: kind,
      sessionID: sessionID,
      turnID: turnID,
      cwd: cwd,
...
final class CodexLocalSessionObserver: @unchecked Sendable {
  ...
  private struct FileState {
    ...
    var offset: UInt64
    var fragment = Data()
    var activeEvent: CodexObservedLifecycleEvent?
  ...
  private let handler: @Sendable (CodexObservedLifecycleEvent) -> Void
  private let pinnedThreadsDidChange: @Sendable () -> Void
  private let now: @Sendable () -> Date
  private let queue = DispatchQueue(
    label: "com.boringnotch.CodexLocalSessionObserver", qos: .utility)
  ...
  init(
    ...
    handler: @escaping @Sendable (CodexObservedLifecycleEvent) -> Void
  ) {
    ...
    guard
      let created = FSEventStreamCreate(
        ...
        { _, info, count, eventPaths, eventFlags, _ in
          ...
          let observer = Unmanaged<CodexLocalSessionObserver>.fromOpaque(info).takeUnretainedValue()
  ...
  private func processInitialFile(
    ...
    var latestEvent: CodexObservedLifecycleEvent?
  ...
  private func apply(_ event: CodexObservedLifecycleEvent, state: inout FileState) {
    handler(event)
    switch event.kind {
    case .working:
  ...
  private func scheduleStale(_ event: CodexObservedLifecycleEvent) {
    cancelStale(for: event.sessionID)
    let staleEvent = CodexObservedLifecycleEvent(
      kind: .stale,
      sessionID: event.sessionID,
      turnID: event.turnID,
      cwd: event.cwd,
