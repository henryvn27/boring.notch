...
enum IntegrationSelfTestError: LocalizedError, Equatable {
  ...
  var errorDescription: String? {
    ...
    case .launchFailed:
      "boring.notch could not launch its installed helper."
    case .timedOut:
      "The helper could not reach boring.notch before the self-test timed out."
    case .processFailed:
      "The helper could not reach boring.notch. Repair the integration and try again."
    case .outputReadFailed:
      "boring.notch could not read the helper's self-test response."
    case .responseTooLarge:
      "The helper returned more self-test data than boring.notch accepts."
    case .malformedResponse:
  ...
  func sendDemo(_ event: IntegrationDemoEvent, sessionID: String) async throws {
    ...
    try await runBoundedProcessOperation {
      let response = try Self.run(
        ...
        timeout: timeout,
        environment: ["BORING_NOTCH_DEMO_SESSION_ID": sessionID]
      )
      let object = try Self.decodeObject(response)
