...
enum CodexHookTrustState: Equatable, Sendable {
  ...
  var summary: String {
    ...
    case .incomplete: "boring.notch hooks are missing or disabled"
...
enum CodexHookTrustServiceError: LocalizedError, Equatable {
  ...
  var errorDescription: String? {
    ...
    case .responseTooLarge: "Codex returned more hook data than boring.notch accepts."
    case .unavailable(let message): message
    }
  }
}
...
struct CodexHookTrustService: Sendable {
  ...
  init(
    ...
    let shim = homeDirectory.appendingPathComponent(".local/bin/boring-notch-hook").path
  ...
  static func parseResponse(
    ...
    let boringNotchHooks = entry.hooks.filter { $0.command == expectedCommand }
    var statuses: [String: String] = [:]
    for event in expectedEvents {
      if let hook = boringNotchHooks.first(where: { $0.eventName == event }) {
