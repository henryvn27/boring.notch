...
enum CodexHookInstallerError: LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .bundledHelperMissing: "The bundled Boring Notch hook helper is missing."
    case .installedHelperUnavailable:
      "The installed Boring Notch helper is unavailable. Repair Codex integration first."
    case .automaticHelperRefreshUnavailable:
      "Run this self-test from Boring Notch installed in /Applications or ~/Applications."
    case .integrationMutationUnavailableDuringUITesting:
      "Codex integration cannot be changed during Boring Notch UI testing."
    case .shimConflict:
      "An integration helper shim already exists and is not Boring Notch's owned symlink. Move it aside before changing the integration."
    case .helperConflict:
      "The installed helper path is not the bundled Boring Notch helper. Move it aside before changing the integration."
...
struct CodexHookInstaller {
  ...
  init(
    ...
    self.bundledHelperURL =
      bundledHelperURL
      ?? applicationBundleURL.appendingPathComponent("Contents/XPCServices/BoringNotchXPCHelper.xpc/Contents/MacOS/BoringNotchXPCHelper")
    ...
    shimURL = homeDirectory.appendingPathComponent(".local/bin/boring-notch-hook")
    installedHelperURL = homeDirectory.appendingPathComponent(
      "Library/Application Support/BoringNotch/bin/boring-notch-hook")
    cowlickShimURL = homeDirectory.appendingPathComponent(".local/bin/cowlick-hook")
  ...
  func status() -> CodexHookInstallationStatus {
    do {
  ...
  func installOrRepair() throws {
    guard allowsIntegrationMutation else {
      throw CodexHookInstallerError.integrationMutationUnavailableDuringUITesting
    }
  ...
  func currentInstalledHelperURL() throws -> URL {
    guard allowsAutomaticHelperRefresh else {
      throw CodexHookInstallerError.automaticHelperRefreshUnavailable
    }
    return try withIntegrationLock {
      guard hasOwnedShim else { throw CodexHookInstallerError.installedHelperUnavailable }
      try installBundledHelper()
      guard helperExists else { throw CodexHookInstallerError.installedHelperUnavailable }
  ...
  static func allowsAutomaticHelperRefresh(
    ...
    let recognizedLocations = [
      URL(fileURLWithPath: "/Applications/boringNotch.app", isDirectory: true),
      homeDirectory.appendingPathComponent("Applications/boringNotch.app", isDirectory: true),
    ]
  ...
  static func merging(
    ...
    if root["hooks"] != nil, !(root["hooks"] is [String: Any]) {
      throw CodexHookInstallerError.invalidHooksObject
    }
  ...
  private func validateIntegrationRemoval() throws {
    if pathExistsWithoutFollowingSymlinks(shimURL), !hasOwnedShim {
      throw CodexHookInstallerError.shimConflict
    }
    if pathExistsWithoutFollowingSymlinks(installedHelperURL) {
      guard installedHelperMatchesBundle else {
        throw CodexHookInstallerError.helperConflict
      }
    }
    try validateLegacyHelperRemoval()
  }
  ...
  private func installBundledHelper() throws {
    guard fileManager.fileExists(atPath: bundledHelperURL.path) else {
      throw CodexHookInstallerError.bundledHelperMissing
    }
        ...
        andPath: bundledHelperURL.path)
    {
      let temporaryHelper = installedHelperURL.deletingLastPathComponent()
        .appendingPathComponent(".boring-notch-hook-\(UUID().uuidString).tmp")
      ...
      guard rename(temporaryHelper.path, installedHelperURL.path) == 0 else {
        throw CodexHookInstallerError.helperReplacementFailed(errno)
      }
    }
  ...
  private func atomicWrite(_ data: Data, to destination: URL, expectedOriginal: Data?) throws {
    ...
    let temporary = destination.deletingLastPathComponent()
      .appendingPathComponent(".hooks.json.boring-notch-\(UUID().uuidString).tmp")
    let descriptor = Darwin.open(temporary.path, O_WRONLY | O_CREAT | O_EXCL, 0o600)
    guard descriptor >= 0 else { throw CodexHookInstallerError.atomicWriteFailed(errno) }
    ...
    guard writeSucceeded else {
      ...
      throw CodexHookInstallerError.atomicWriteFailed(errno)
    }
    ...
    guard rename(temporary.path, destination.path) == 0 else {
      let code = errno
      unlink(temporary.path)
      throw CodexHookInstallerError.atomicWriteFailed(code)
    }
  }
  ...
  private static func validateSupportedHookShapes(in root: [String: Any]) throws {
    ...
    for event in supportedEvents where hooks[event] != nil {
      ...
      for rawGroup in groups {
        ...
        else { throw CodexHookInstallerError.invalidHooksObject }
      }
    }
  }
  ...
  private static func encodeAndValidate(_ root: [String: Any]) throws -> Data {
    let data =
      ...
      + Data([0x0A])
    do { _ = try decodeRoot(data) } catch { throw CodexHookInstallerError.validationFailed }
    return data
  }
  ...
  private static func isCanonicalHandler(
    ...
    guard handler["type"] as? String == "command",
      ...
      handler["statusMessage"] as? String == "Boring Notch",
      let marker = handler["boringNotch"] as? [String: Any],
      marker["product"] as? String == "Boring Notch",
      marker["protocol"] as? Int == CodexBridgeEvent.currentVersion
    else { return false }
    return true
  }
  ...
  private static func isCurrentOwnedHandler(
    ...
    if let marker = handler["boringNotch"] as? [String: Any],
      marker["product"] as? String == "Boring Notch"
    {
      return true
    }
  ...
  private static func canonicalHandler(event: String, command: String) -> [String: Any] {
    [
      ...
      "statusMessage": "Boring Notch",
      "boringNotch": ["product": "Boring Notch", "protocol": CodexBridgeEvent.currentVersion],
    ]
  }
  ...
  private func withIntegrationLock<Result>(_ body: () throws -> Result) throws -> Result {
    ...
    let lockURL = hooksURL.deletingLastPathComponent()
      .appendingPathComponent(".hooks.json.boring-notch.lock")
  ...
  private func readHooksDataIfPresent() throws -> Data? {
    ...
    guard lstat(hooksURL.path, &pathInformation) == 0 else {
      ...
      throw CodexHookInstallerError.unsafeHooksFile
    }
    guard pathInformation.st_mode & S_IFMT == S_IFREG,
      pathInformation.st_uid == getuid()
    else { throw CodexHookInstallerError.unsafeHooksFile }

    let descriptor = Darwin.open(hooksURL.path, O_RDONLY | O_NOFOLLOW)
    guard descriptor >= 0 else { throw CodexHookInstallerError.unsafeHooksFile }
