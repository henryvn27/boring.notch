...
struct CodexPersistedLifecycleSession: Codable, Equatable, Sendable {
...
enum CodexLifecycleLedger {
  ...
  static func load(
    ...
    now: Date = Date()
  ) -> [CodexPersistedLifecycleSession] {
  ...
  private struct LedgerFile: Codable {
    let version: Int
    let sessions: [CodexPersistedLifecycleSession]
  }

  private static func mutate(
    homeDirectory: URL,
    now: Date,
    change: (inout [CodexPersistedLifecycleSession]) -> Void
  ) throws {
  ...
  private static func active(
    _ sessions: [CodexPersistedLifecycleSession],
    now: Date
  ) -> [CodexPersistedLifecycleSession] {
  ...
  private static func supportDirectory(homeDirectory: URL) -> URL {
    homeDirectory.appendingPathComponent("Library/Application Support/BoringNotch", isDirectory: true)
  }
